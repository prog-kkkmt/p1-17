...
void to_html(vector <City>& v)
{
	ofstream out("city.html");
	...
	out << "<tr>" << endl << "<th>code city</th>" << endl << "<th>distance</th>" << endl << "<th>code rate</th>" << endl;
	for(int i = 0; i < v.size(); i++)
	{
		out <<"<tr>"<< endl;
		out << "<td>" << v.at(i).codeC << "</td><td>"<< v.at(i).dist <<"</td><td>"<< v.at(i).codeR<< "</td>" << endl;
		out << "</tr>" << endl;
	}
