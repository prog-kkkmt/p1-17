...
struct Currency {
    ...
    void HTMLrow(ofstream &os) {
        os << "<td>" << ID << "</td><td>" << name << "</td><td>" << course << "</td>";
    }
};
...
struct Customer {
    ...
    void HTMLrow(ofstream &os) {
        os << "<td>" << ID << "</td><td>" << name << "</td><td>" << passport << "</td>";
    }
};
...
struct Deal {
    ...
    void HTMLrow(ofstream &os) {
        os << "<td>" << ID << "</td><td>" << IDCustomer << "</td><td>" << IDCurrency << "</td><td>" << amount << "</td>";
    }
};
...
void outputHTML(vector <T> cur, string filename) {
    ...
    if (out.is_open())
    {
        ...
        out << "<title>Таблица</title>"<< endl << "</head>" << endl << "<body>" << endl;
        out << "<table border='1' cellspacing='0'>";
        for (int i = 0; i < cur.size(); i++) {
            out << "<tr>" << endl;
            cur.at(i).HTMLrow(out); 
            out << endl;
            out << "</tr>" << endl;
        }
