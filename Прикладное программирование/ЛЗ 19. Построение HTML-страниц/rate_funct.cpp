...
void to_html(vector <Rate>& v)
{
	ofstream out("rate.html");
	out << "<html>" << endl << "<head>" << endl;
	out << "<meta http-equiv=""Content-Type"" content=""text/html; charset=utf-8"">" << endl;
	out << "<title>table</title>" << endl << "</head" << endl << "<body>" << endl;
	out << "<table border='1' width=""30%"" cellspacing='3'>";
	out << "<tr>" << endl << "<th>code rate</th>" << endl << "<th>min distance</th>" << endl << "<th>max distance</th>" << endl << "<th>price</th>" << endl;
	for(int i = 0; i < v.size(); i++)
	{
		out <<"<tr>"<< endl;
		out << "<td>" << v.at(i).codeR << "</td><td>" << v.at(i).mindist << "</td><td>"<< v.at(i).maxdist <<"</td><td>"<< v.at(i).price<< "</td>" << endl;
		out << "</tr>" << endl;
	}
