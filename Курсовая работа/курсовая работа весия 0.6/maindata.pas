...
begin                  //поочереди включаем все необходимые нам компоненты
     ...
     if(opendialog1.Execute) then begin
                             ...
                             IDData.DatabaseName:='127.0.0.1:' + ExtractFileDir(OpenDialog1.FileName)+'\MYSTUDENTS.FDB';    //записываем путь в дату
                             IDData.Connected:=true;            //подключаем
                             IdTrans.StartTransaction;       //начинаем транзакцию
                             MainGroups.open;       //далее открываем все запросы и таблицы
                             QueryStudents.Open;
                             MainStudent.Open;
                             quaryNameGR.open;
                             workstd.open;
