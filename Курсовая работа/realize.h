...
void student :: PrintAllInformation(size_t k){
    std::cout<<"Главная информация о студенте "<<name<<"     "<<second_name<<"     "<<father_name<<"     "<<KolAbsenteeisms<<"     "<<Estimation<<std::endl;
    std::cout<<"Прогулы: ";
...
std::fstream& operator << (std::ofstream &os, const group &person){
	os<<"Дни- "<<person.day<<" .Студенты- "<<person._size<<std::endl;
	for(size_t i =0; i<person._size; i++){
		os<<"ФИО: "<<person.persons[i].name<<" "<<person.persons[i].second_name<<" "<<person.persons[i].father_name<<std::endl;
		os<<"Прогулы: ";
