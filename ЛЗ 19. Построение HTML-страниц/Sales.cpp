...
void Sales::printToHtml(ofstream &f){
    f << "<H1>Продажи</H1>"<< endl;
	f << "<table border = \"2 \">" << endl;
    f << "<tr id = \"header\">" << endl;
	f << "<td><i>Код менеджера</i></td>" << endl;
	f << "<td><i>Код автомобиля</i></td>" << endl;
	f << "<td><i>Государствeнный номер</i></td>" << endl;
	f << "<td><i>Код покупателя</i></td>" << endl;
	f << "<td><i>Дата</i></td>" << endl;
	f << "<td><i>Цена</i></td>" << endl;
	f << "</tr>" << endl;
    for (unsigned i = 0; i < rows.size() ; i++)
    {
        f << "<tr>" << endl;
        f << "<td>" << rows.at(i).getManagerCode() << "</td>" << endl;
        f << "<td>" << rows.at(i).getCarCode() << "</td>" << endl;
        f << "<td>" << rows.at(i).getLicensePlate() << "</td>" << endl;
        f << "<td>" << rows.at(i).getBuyerCode() << "</td>" << endl;
        f << "<td>" << rows.at(i).getDate() << "</td>" << endl;
        f << "<td>" << rows.at(i).getPrice() << "</td>"<< endl;
        f << "</tr>" << endl;
     }
     f << "</table>" << endl;
}
